...
void *routine(void *arg)
{
	...
	while(1)
	{
		...
		while(pool->waiting_tasks == 0 && !pool->shutdown)	//判断线程池是否没有需要运行的任务并且公司没倒闭	
		{
			pthread_cond_wait(&pool->cond, &pool->lock);	//让线程睡眠		
		}


		if(pool->waiting_tasks == 0 && pool->shutdown == true)	//判断线程池是否没有需要运行的任务并且公司倒闭	
		{
			pthread_mutex_unlock(&pool->lock);	//解锁
			pthread_exit(NULL);					//退出线程					
		}

											//线程池有需要运行的任务并且公司没倒闭，执行以下代码
		p = pool->task_list->next;			//让p登记需要运行的任务节点	（拿走一个线程池里的任务，以头删的方式，因为是任务队列，队列的特点是->头删尾插）					
		...
		pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);		//忽略线程的取消操作（在执行线程任务的时候，不响应线程取消）
		(p->task)(p->arg);											//函数调用	
		free(p);													//释放任务节点									
			...
			void *(*task)(void *arg), void *arg)
{
	...
	pthread_mutex_unlock(&pool->lock);					//解锁
	pthread_cond_signal(&pool->cond);					//唤醒正在睡眠中的线程（当所有的员工（线程）都有任务时，唤醒一下无伤大雅）
...
int remove_thread(thread_pool *pool, unsigned int removing_threads)
{
	...
	int i;
	for(i=pool->active_threads-1; i>remain_threads-1; i--)			//从id的最后一位线程开始取消（这里的active_threads-1,是因为下面要用他来做线程数组下标）
	{
...
bool destroy_pool(thread_pool *pool)
{
	...
	{
		errno = pthread_join(pool->tids[i], NULL);//等待线程退出，有阻塞效果（当某个线程陷入死循环时，main主线程会堵塞在这里）
		if(errno != 0)
		{
