...
int display_jpeg_picture_to_framebuffer(const char *filename, unsigned int x_s, unsigned int y_s, unsigned int *screen_fb_addr, float mul)
{
    ...
    while (cinfo.output_scanline < cinfo.output_height)
    {
        ...
        */
        for (x = x_s, x_data_offset = point_distance; x < x_s + cinfo.output_width * mul; x++, x_data_offset += point_distance)
        {
            ...
            display_point_to_framebuffer(x, y, color, screen_fb_addr);
...
int display_camera_to_screen(unsigned int x_s, unsigned int y_s, void *jbuffer, unsigned long jlen, unsigned int *screen_fb_addr, float mul)
{
    ...
    jpeg_mem_src(&cinfo, jbuffer, jlen);
    ...
    buffer = (char *)(*cinfo.mem->alloc_sarray)((j_common_ptr)&cinfo, JPOOL_IMAGE, row_stride, 1);
